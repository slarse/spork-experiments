#! /bin/bash
# Script for setting up the experiments on Ubuntu 18.04. Making this work for
# other distributions will likely require some amount of work.
#
...
# that is used for other activites.

REPLICATION_PKG_URL="https://github.com/KTH/spork/releases/download/v0.5.1/replication_package.tar.gz"
...
function build_jdime() {
    clone_dir="$REPLICATION_PKG_DIR/replication/software/jdime"
    git clone https://github.com/se-sic/jdime.git "$clone_dir" \
        && cd "$clone_dir" \
...
function build_automergeptm() {
    clone_dir="$REPLICATION_PKG_DIR/replication/software/automergeptm"
    git clone https://github.com/thufv/automerge-ptm.git "$clone_dir" \
        && cd "$clone_dir" \
        && git checkout e73038b592260ea4f19a266322167487ce152cc5 \
        && ./gradlew installDist \
        && cd - || exit 1
}
